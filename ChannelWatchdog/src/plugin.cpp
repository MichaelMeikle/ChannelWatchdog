...
const char* ts3plugin_name() {
	return plugin->plugin_name().c_str();
}

/* Plugin version */
const char* ts3plugin_version() {
    return plugin->plugin_version().c_str();
}
...
const char* ts3plugin_author() {
    ...
    return plugin->plugin_author().c_str();
}

/* Plugin description */
const char* ts3plugin_description() {
	/* If you want to use wchar_t, see ts3plugin_name() on how to use */
    return plugin->plugin_description().c_str();
}
...
void ts3plugin_onMenuItemEvent(uint64 serverConnectionHandlerID, enum PluginMenuType type, int menuItemID, uint64 selectedItemID) {
	plugin->MenuEvent(serverConnectionHandlerID, type, menuItemID, selectedItemID);
}
